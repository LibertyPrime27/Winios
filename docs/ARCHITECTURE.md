...
- **D3D12 (modern titles, and the model for the rest):** **d12mt** — our own, public, MIT, vendored at `gpu/d12mt` ([repo](https://github.com/LibertyPrime27/d12mt)). Root signatures become Metal argument-buffer layouts; DXIL becomes MSL through dxil-spirv → SPIRV-Cross with remappers that follow that layout. The compiler and planner exist for all three shader generations (SM3 via dxbc-spirv for D3D9, SM4/5 via dxbc-spirv for D3D11, DXIL via dxil-spirv for D3D12), CI compiles the generated MSL for iOS with Apple's Metal compiler, and the on-device probe passed 27/27 binding checks on the M3 iPad — the descriptor-heap-as-argument-buffer design is confirmed on hardware. The runtime (command lists, fences, PSOs, swap chain) is the next piece; the mapping is written up in `gpu/d12mt/docs/D3D12-ON-METAL.md`.

...
| **dynarec** — x86 basic blocks → ARM64 code, block chaining, SSE/SSE2 on NEON, lazy flags, callouts to the interpreter for the rest | [`docs/DYNAREC.md`](docs/DYNAREC.md), [`core/src/jit/`](core/src/jit) — **passes all 2364 silicon vectors on the M3 iPad** (and under qemu-aarch64 / Apple-silicon CI); JIT-vs-interpreter differential over every difftest case, 63 000 runs identical; n-body 54× the interpreter, busybox sha256 with zero SSE callouts |

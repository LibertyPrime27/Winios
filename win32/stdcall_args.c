/* Generated by tools/gen/stdcall_args.sh -- do not edit.
 *
 * How many bytes of arguments a stdcall function pops on x86, for the
 * Win32 functions we do not implement. An import table gives a name and
 * nothing else; without this, returning from an unimplemented import would
 * either corrupt the stack or not be possible at all, and the first missing
 * function would end every run. With it, the call is logged, zero is
 * returned, the stack is left correct, and the program carries on to show
 * what else it needs.
 *
 * Lifted from mingw-w64's import libraries, which decorate stdcall imports
 * as _Name@bytes. 9817 functions, sorted, binary-searched.
 */
#include <stdint.h>
#include <string.h>

typedef struct { const char *name; uint16_t bytes; } stdcall_arg;
static const stdcall_arg g_stdcall[] = {

...
typedef struct {
    int  ok;
    char name[256];          /* the library entry: what to call it            */
    char dir_rel[1024];      /* the program's folder, relative to C:\. Usually
                              * the same as `name`, but not when an installer
                              * put it somewhere structured -- a wizard the
                              * person answered themselves routinely installs
                              * to Program Files\<Game>, and an entry that
                              * assumed the folder was called `name` would
                              * point at nothing.                             */
 ...
 *   visible     WI_SILENT runs the installer in its own silent mode: no
 *               window, no questions, and the destination is the one we
 *               chose. WI_VISIBLE runs it as a person would see it, with its
 *               dialogs drawn and a person answering them -- which is the
 *               only way through an installer that has a licence to accept
 *               or a component to choose, and the only way the person gets
 *               to say where it goes.
 *
 *               Either way the program is found the same way afterwards, by
 *               looking at what appeared on the drive. That matters most for
 *               the visible run: the person may have typed a destination we
 *               never suggested, so where it went is a question only the
 *               drive can answer.
 ...
 *               be allowed to wait forever. A visible run wants a generous
 *               one, because the thing it is waiting for is a person.
